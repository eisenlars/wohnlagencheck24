...
export default async function NetworkPartnerLoginPage({
  searchParams,
}: {
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '100px auto', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', border: '1px solid #ddd', padding: '24px', borderRadius: '8px' }}>
        <h2 style={{ margin: '0 0 10px 0', color: '#111827' }}>Netzwerkpartner Login</h2>
        <p style={{ fontSize: '14px', color: '#666' }}>Anmeldung für den getrennten Self-Service-Bereich der Netzwerkpartner.</p>
        ...
        <form action={login} style={{ display: 'grid', gap: '12px' }}>
          ...
          <FormSubmitButton
            ...
            style={{ padding: '10px', background: '#0f766e', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 700 }}
            spinnerColor="#ffffff"
          />
        </form>
            ...
            __html: `
              (function () {
                try {
                  ...
                  if (hash.indexOf("type=invite") !== -1 || hash.indexOf("type=recovery") !== -1) {
                    window.location.replace("/auth/setup?aud=network_partner" + hash);
                  }
                } catch (_) {}
              })();
            `,
        ...
        {message ? (
          <p
            style={{
              color: isSuccessMessage ? '#166534' : '#b91c1c',
              fontSize: '14px',
              textAlign: 'center',
            }}
          >
