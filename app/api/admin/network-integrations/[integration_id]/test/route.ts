...
async function patchIntegrationTestState(
  ...
  const prev =
    data && typeof data === "object" && !Array.isArray(data) && typeof (data as Record<string, unknown>).settings === "object"
      ? asObject((data as Record<string, unknown>).settings)
      : {};
...
export async function POST(
  ...
  try {
    ...
    const networkPartner = await getNetworkPartnerById(integration.network_partner_id);
    if (!networkPartner) {
    ...
    if (integration.kind === "llm" && !networkPartner.llm_partner_managed_allowed) {
      return NextResponse.json({ error: "LLM-Anbindungen sind fuer diesen Netzwerkpartner nicht freigeschaltet." }, { status: 403 });
    }
    ...
    const finish = async (result: TestResultPayload, step: string) => {
      ...
      await patchIntegrationTestState(integration.id, integration.network_partner_id, {
