-- partner_references_canonical_phase_a
--
-- Ziel:
-- 1) bestehende Tabelle `partner_references` fuer den kuenftigen Canonical-Layer vorbereiten
...
create index if not exists partner_references_partner_live_idx
  on public.partner_references (partner_id, lifecycle_status, is_live);

create index if not exists partner_references_partner_source_updated_idx
  on public.partner_references (partner_id, source_updated_at desc);

create index if not exists partner_references_partner_source_external_idx
  on public.partner_references (partner_id, source, external_id);

create unique index if not exists partner_references_partner_source_external_unique
  on public.partner_references (partner_id, source, external_id)
