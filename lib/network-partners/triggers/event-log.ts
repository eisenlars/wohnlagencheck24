...
export async function createTriggerEventLog(
  ...
  if (error) {
    ...
    throw new Error(String(error.message ?? "TRIGGER_EVENT_CREATE_FAILED"));
  }
...
export async function finishTriggerEventLog(input: {
  id: string | null;
  ...
  if (error) {
    ...
    throw new Error(String(error.message ?? "TRIGGER_EVENT_UPDATE_FAILED"));
  }
}
