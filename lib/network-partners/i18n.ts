...
import { getContentByIdForPortalPartner } from "@/lib/network-partners/repositories/content";
...
export async function listNetworkContentTranslations(
  ...
  return (data ?? []).map((row) => mapTranslationRow(row as Record<string, unknown>));
}
